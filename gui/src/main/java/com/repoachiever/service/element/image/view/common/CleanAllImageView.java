...
import com.repoachiever.service.event.payload.CleanAllEvent;
...
public class CleanAllImageView implements IElementResizable, IElement<BorderPane> {
  ...
  public CleanAllImageView(
    ...
    button.setOnMouseClicked(
        event -> applicationEventPublisher.publishEvent(new CleanAllEvent()));

    InputStream imageSource =
        getClass().getClassLoader().getResourceAsStream(properties.getImageCleanAllName());
    ...
    splitPane.setTooltip(new Tooltip(properties.getButtonCleanAllDescription()));
